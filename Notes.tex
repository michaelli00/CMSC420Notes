...
\begin{document} 
  ...
  \end{lstlisting}
  \textbf{Sequential Allocation} (Array): when array is full, increase its size by a constant factor (e.g. 2). Amortized array operations still O(1) \\
  \textbf{Linked Allocation} (Linked List) \\ \\
  Arrays and LinkedLists can be used to create:
  \begin{itemize}[noitemsep,topsep=0pt]
    \item \textbf{Stack(push, pop)}: on on end of the list
    \item \textbf{Queue(enqueue, dequeue)}: insert at tail (end) and remove from head (start)
    \item \textbf{Deque(combo stack and queue)}: can insert and remove from either ends of list
    \item \textbf{Multilist}: multiple lists combined into one aggregate structure (e.g. ArrayList)
    \item \textbf{Sparse Matrix}: create a multilist of 2n linked lists for each row and col
    \setlist{nolistsep}
  ...
  \end{center}
  \subsection{Proof Amortized Cost for Sequential Allocation}
  \textbf{Theorem:} When doubling array size for reallocation, amortized cost of list operation is $O(1)$.\\ \\
  \textbf{Proof:} Assume we are using a stack (can be proven for other ADT). Say we have $n+1 elements$ and Take a specific run between reallocations (up until $2n+1$ elements). \\
  Each time we push an element, we pay 1 token for push and store an additional 4 tokens into a bank. Once the array is full, we reallocate an array of size $2n$ which requires intializing the array and copying elements (this will take $2*2n=4n$ operations). \\
  Since we payed 4 tokens over n insert operations during this run, we have $4n$ tokens which pay for the reallocation cost. \\ \\
  ...
  Trees are represented as graphs $G = (V, E)$ where $V$ is a set of vertices and $E$ is a set of edges\\
  \textbf{Free Tree}: connected, undirected graph with no cycles (like MST)\\
  \textbf{Root Tree}: each non-leaf node has $\geq$ 1 children and a single parent (except root)
  \setlist{nolistsep}
  \begin{itemize}[noitemsep]
  ...
  \textbf{Binary representation}: nodes have a pointer to first child and another pointer to next sibling
  \begin{center}
    \includegraphics[scale=0.3]{Fig_3}
  \end{center}
  \textbf{Binary Tree}: rooted, ordered tree where each non-leaf node has 2 possible children (left, right)
  \setlist{nolistsep}
  \begin{itemize}[noitemsep]
    \item \textbf{Full Tree}: All nodes either have 0 children or 2 children
  ...
  If there are $n$ internal nodes, there are $n + 1$ external nodes. Proof by noticing that we will essentially replace every leaf with 1 internal and 2 external nodes whenever we try to add a new internal node to extended tree.\\  \\
  \textbf{Theorem: }If there are n internal nodes in an extended tree, there are n+1 external nodes\\
  \textbf{Proof by Induction: } 
  \begin{itemize}[noitemsep]
  \item Extended tree binary tree with n internal nodes has n+1 external nodes has 2n+1 total nodes
  ...
  \begin{center}
    balance(v) = height(v.right) - height(v.left) with height(null) = -1
  \end{center}
  ...
  After insertion, the height of the modified subtree is the same as it was before the insertion which implies we don't need anymore rotations \\
  If a node has a balance factor that violates Balance Property:
  \begin{itemize}[noitemsep]
  \item Left-Left subtree too deep then rotate right on unbalanced node
  ...
  \textbf{Lazy Delete}: Mark each node as dead or alive. If node is deleted, it is dead. If a node is added to a position where a dead node used to be, flip the node to alive and replace fields.
  \begin{itemize}[noitemsep, topsep=0pt]
    \item Issue of having too many dead nodes (can perform garbage collection if too many dead nodes)
  \end{itemize}
  \newpage 
  ...
  \begin{itemize}[noitemsep]
  \item empty (null) with h = -1
  \item root is 2-node and has two 2-3 subtrees of equal height $h-1$
  \item root is 3-node and has three 2-3 subtrees of equal height $h-1$
  \end{itemize}
  Sparsest 2-3 tree is a complete binary tree and therefore has a height of $O(logn)$\\ \\
  ...
  \item Also can't convert a node with 2 red children to 2-3 tree which ends up being a 4-node (RB trees have a 1-1 mapping with 2-3-4 trees)
  \end{itemize}
  \begin{center}
  ...
  \begin{itemize}[noitemsep]
    ...
    \item \textbf{split(p)} if p has a red right-right chain, do a left rotation \& promote first red child to next level (this promoted node becomes red node)
  \end{itemize}
  ...
  \textbf{Geomtric Interpretation}: create a rectangles using priority number as horizontal borders and key values as vertical borders.
  \begin{center}
  \includegraphics[scale=0.3]{TreapRep}
  \end{center}
  ...
  \item node has 2 children then set its priority to $\infty$ and apply rotations to sift down to leaf and remove or can just do normal unbalanced binary tree delete
  \end{itemize}
  \begin{center}
  ...
  \begin{lstlisting}
    private class TreapNode {
      Key key // key
      Value value // value
      int priority // random priority (set when node is created)
      TreapNode left // left child
      TreapNode right // right child
    }

    // utility functions
    int getPriority(TreapNode p) { return (p == null ? MAX_PRIORITY : p.priority) } //null nodes have priority of infinity
    ...
    TreapNode insert(Key x, Value v, TreapNode p) {
      ...
      p = new TreapNode(x, v, null, null) // ... create a new leaf node here
    ...
    TreapNode delete(Key x, TreapNode p) {
      if (p == null) Error - Nonexistant key! // key not found
      else {
        if (x < p.key) p.left = delete(x, p.left) // delete from left
          ...
          if (p.left == null) return p.right // return the other child
          else return p.left
        } else { // both children present
          TreapNode r = findReplacement(p.right) // find replacement node
          copy contents from r to p
  ...
  Ideal skip lists made up of multiple levels that are built from
  \begin{itemize}[noitemsep]
  ...
  \end{itemize}
  Issue with ideal skip list being too pure since adding a node in the middle of the list will trigger resizing everything\\ \\
  \textbf{Randomized Skip List:} For each node flip a coin. If it lands head raise it one level and flip another coin. If it's tails leave it where it is\\ \\
  \textbf{Implementation: }
  \begin{lstlisting}
  class SkipNode {
    Key key
  ...
  \textbf{Height Analysis: } expected number of levels is $O(logn)$\\
  Probability that one of n elements is at height $l$: $\frac{n}{2^{l}}$ \\
  Now let $l = clgn \rightarrow \frac{n}{2^{l}} = \frac{n}{n^{c}} = n^{1-c}$ so likelihood that $l$ exceeds $clgn$ is very slim\\
  ...
  $E(i) = 1 + p \dot E(i-1) + (1-p) \dot E(i) \rightarrow E(i) = \frac{1}{p} + E(i-1) = \frac{i}{p}$ by recurrence analysis and since i $\leq$ O(logn) then search is O(logn)\\ \\
  ...
  \textbf{Proof 1: }assume m is even and let N(h) = number of nodes in skinniest possible order-m B tree of height h\\
  ...
  \textbf{Proof 2: }\\
    \indent let $N(h)$ denote smallest number of keys in the leaves of the tree \\
    \indent root has at least 2 keys and all other nodes have $d$ keys so last row of keys has $N(h) \geq 2d^{h} \geq d^{h}$ keys yielding $\frac{lg(N(h))}{lgd} \geq h$
  ...
  \textbf{Rotation (Adoption): }Node can have between $\ceil[\Big]{m/2}$ and $m$ children, and one less keys. Insertion and deletion might make a node have too many or too few nodes so we fix this imbalance by moving a child into or from one of its siblings and rotate a key from parent, assuming it can give a child\\
  \begin{center}
...
\item \textbf{Polynomial Hashing: }$h(x_{0}, ..., x_{n}) = (\sum_{i=0}^{k-1}c_{i}p^{i}) \mod m$
  ...
  max height is $log_{3/2}m \leq log_{3/2}(2n)$
  \begin{itemize}[noitemsep]
...
\begin{center}
  ...
  \textbf{Find:} $O(log(n))$ because tree height is $O(log(n))$ \\
  \textbf{Delete:} Token analysis can prove that $O(n)$ cost of rebuilding entire tree can be amortized against non expensive deletes\\
  \textbf{Insert:} Token analysis can prove that $O(k)$ cost of rebuilding tree can be amortized against non expensive inserts
  \newpage
